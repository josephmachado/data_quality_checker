...
def test_check_unique(mock_get_checker):
    ...
    result = runner.invoke(
        app, ["check-unique", "--data", "users.csv", "--column", "user_id"]
    )
...
def test_check_not_null(mock_get_checker):
    ...
    result = runner.invoke(
        app, ["check-not-null", "--data", "users.csv", "--column", "age"]
    )
...
def test_check_enum(mock_get_checker):
    ...
    assert result.exit_code == 0
    assert (
        "Column 'status' in 'users.csv' contains only allowed values" in result.stdout
    )
    mock_checker.is_column_enum.assert_called_once_with(
...
def test_check_references(mock_get_checker):
    ...
    mock_checker.are_tables_referential_integral.assert_called_once_with(
        "orders.csv", "users.csv", ["user_id"]
    )
...
def test_check_column_exists(mock_get_checker):
    ...
    result = runner.invoke(
        app, ["check-column-exists", "--data", "users.csv", "--column", "email"]
    )
